<!DOCTYPE html>
<html lang = "en">
	<head>
		<meta charset = "utf-8"/>
		<title>Milestone 2</title>
	</head>
	<body>
		<h1>Content Strategy</h1>
		<h2>Intro</h2>
		<p>Hey! I'm Max!</p>
	<h2>Description</h2>
		<p>I specialize in responsive web development, Wordpress, and Swiss style when it comes to design </p>
		<h2>Skills</h2>
		<p>PHP, JAVASCRIPT, CSS HTML</p>
		<h2>Contact Me</h2>
		<p>This will be a contact form to submit</p>
		<h2>Graphic Content</h2>
		<p>Logo,Profile Pic, Animation Photo and Background Photo as well as Speech Bubble.</p>
		<h3>Purpose of Content</h3>
		<p>The purpose of this content is to give the visitor a clear respresentation of the focus and skills that Max has to offer as a developer.</p>
		<h2>Wireframes</h2>
		<img src ="images/Wirefram1PWP.png" alt="Wireframe 1"/>
		<img src ="images/Wireframe2PWP.png" alt="Wireframe2"/>
	</body>
</html>